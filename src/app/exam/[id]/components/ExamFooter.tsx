...
export default function ExamFooter({ 
  ...
  return (
    <footer 
      className="bg-blue-100 p-2 flex justify-between items-center relative z-20 select-none"
      style={{ 
    ...
    >
      {/* Left: Student Name */}
      <div className="pl-4">
          <p className="text-gray-800 text-[15px]">{studentName}</p>
      </div>

      {/* Center: Question Navigation Pill */}
      <div 
          className="bg-black text-white px-4 py-2 flex items-center gap-2 rounded-xl cursor-pointer absolute left-1/2 transform -translate-x-1/2"
          onClick={() => setIsModalOpen(!isModalOpen)}
      >
          <p className="text-white text-[15px] whitespace-nowrap">Question {currentQuestionInModule} of {moduleQuestions.length}</p>
      ...
      {/* Question Navigation Modal (The grid overlay) */}
      {isModalOpen && (
          <div className="absolute bottom-full mb-4 w-[600px] bg-white rounded-xl shadow-2xl border border-gray-200 p-6 z-50 transform left-1/2 -translate-x-1/2 text-black">
              {/* Pointer */}
              ...
              <div className="flex justify-between items-start mb-6">
                  <h3 className="text-lg font-bold font-serif">{sectionTitle}</h3>
